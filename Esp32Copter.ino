...
#ifdef flysky //若 flysky 有被定義,執行下列程式
  #define ROL 0
  #define PIT 1
  #define THR 2
  #define RUD 3
#else //若 flysky 沒被定義,執行下列程式 //orangerx
  #define ROL 1
  #define PIT 2
  #define THR 0
  #define RUD 3
...
void setup() 
{
  ...
  EEPROM.begin(64);//宣告使用的空間為 64 bytes
  if (EEPROM.read(63) != 0x55) //讀入並判斷資料是否=85(decimal)
  Serial.println("Need to do ACC calib");
  else ACC_Read(); // eeprom is initialized
  if (EEPROM.read(62) != 0xAA) //讀入並判斷資料是否=170(decimal)
  ...
  //以工作站（Station）模式啟動，ESP32用來上網讀取資料，此為預設模式
    ...
    init_RC();
  #else
    Serial.printf("This mac: %s, ", WiFi.macAddress().c_str()); //取得nodeMCU的mac address
  ...
  delay(500); 
  pinMode(LED,OUTPUT); //在初始化的時候，設定特定的腳位作為輸出
  digitalWrite(LED,LOW); //降低電壓到0V，來關閉LED
  initServo();
}

uint32_t rxt; // receive time, used for failsave
...
//micros():Returns the number of microseconds since the Arduino board began running the current program
  ...
  // parser part
  if (Serial.available())
  //Serial.available():The number of bytes available to read
  {
    char ch = Serial.read();//讀取從序列埠輸入的英文字元
    // Perform ACC calibration
    if (ch == 10) Serial.println();//it returns the number of bytes written
    else if (ch == 'A')
    { 
